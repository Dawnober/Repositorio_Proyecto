...
// Se recomienda mapear el controlador a la URL principal que carga el dashboard
...
public class AdminControlador extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		// --- 1. GESTIÓN DE SEGURIDAD (Obligatorio) ---
		HttpSession session = request.getSession(false);
		// Si la sesión es nula o no está logueado, redirigir al inicio.
		if (session == null || session.getAttribute("isLoggedIn") == null || 
			!(Boolean)session.getAttribute("isLoggedIn")) {
			// Usar getContextPath() para asegurar la ruta correcta de redirección
			response.sendRedirect(request.getContextPath() + "/index.jsp"); 
			return;
		}

        // 🚨 NOTA IMPORTANTE DE LA CORRECCIÓN:
        // Se ha ELIMINADO la lógica temporal para establecer 'userRolDisplay' y 'userName'.
        // Estas variables DEBEN ser establecidas en el Servlet de Login (LoginProcesador.java)
        // para que estén disponibles inmediatamente en la primera carga del dashboard.
        // Si no aparecen, la corrección debe aplicarse en el LoginProcesador.java.
        
		// --- 2. DETERMINAR EL MÓDULO (Vista) A CARGAR ---
		// El parámetro 'view' viene de los enlaces de tu menú lateral.
		...
		// Esto asegura que el contenido de Almacén/Bienvenida cargue inmediatamente al inicio (como se ve en tus imágenes).
		...
		if (moduloActual.equals("almacen1")) { 
			// 🚨 Aquí va la LÓGICA para obtener los datos de la base de datos (BD) 🚨
		...
		// **Puedes replicar el 'else if' para cada módulo que requiera datos de la BD**
		/*
	...
	// Pero para la navegación del menú (GET) solo necesitamos el doGet.
}